  ...
  return (
    <AppShell>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-4">
        <Stat label="Streak" value={overview ? `${overview.streak_days}d` : "—"} />
        <Stat label="Tasks completed" value={overview?.tasks_completed ?? "—"} />
        <Stat label="Interviews" value={overview?.interviews_completed ?? "—"} />
      ...
      <div className="card mt-6">
        ...
        {skills.length === 0 ? (
          <p className="mt-2 text-sm text-slate-500">Complete tasks and interviews to build your skill map.</p>
        ) : (
      ...
      <div className="card mt-6">
        ...
        {history.length === 0 ? (
          <p className="mt-2 text-sm text-slate-500">No interviews yet.</p>
        ) : (
          <table className="mt-3 w-full text-sm">
            <thead>
              <tr className="border-b border-slate-200 text-left text-xs uppercase text-slate-400">
                <th className="py-2">Date</th><th>Type</th><th>Role</th><th>Score</th><th>Signal</th><th></th>
              </tr>
            </thead>
            <tbody>
              {history.map((h) => (
                <tr key={h.session_id} className="border-b border-slate-100">
                  <td className="py-2">{h.ended_at ? new Date(h.ended_at).toLocaleDateString() : "in progress"}</td>
                  ...
                  <td>
                    {h.overall_score !== null && (
                      <Link className="text-brand-600" href={`/interviews/${h.session_id}/report`}>Report</Link>
                    )}
                  </td>
                </tr>
              ))}
