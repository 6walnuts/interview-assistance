...
export default function PracticePage() {
  return (
    <AppShell>
      ...
      <p className="mt-1 text-slate-600">Short, targeted sessions — 5, 15 or 30 minutes.</p>
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {DRILLS.map((d) => (
          <Link key={d.name} href={d.href} className="card transition hover:border-brand-500">
            <div className="flex items-center justify-between">
              <h2 className="font-semibold">{d.name}</h2>
              <span className="rounded bg-slate-100 px-2 py-0.5 text-xs text-slate-500">{d.time}</span>
            </div>
